...
func _update_joystick(mouse_position: Vector2) -> void:
	var _base_radius = _get_base_radius()
	var center : Vector2 = _base.global_position + _base_radius
	var vector : Vector2 = mouse_position - center
	vector = vector.limit_length(clampzone_size)
	
	if joystick_mode == Joystick_mode.FOLLOWING and mouse_position.distance_to(center) > clampzone_size:
		_move_base(mouse_position - vector)
